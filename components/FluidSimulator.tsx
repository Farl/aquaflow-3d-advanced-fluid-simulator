...
const FluidSimulator: React.FC<Props> = ({ config, onStatsUpdate, triggerInject, resetRotation }) => {
  ...
  useEffect(() => {
    ...
    const initialShaderParams = validateShaderParams({
      ...
      waterTintR: configRef.current.waterTintR ?? 0.04,
      waterTintG: configRef.current.waterTintG ?? 0.04,
      waterTintB: configRef.current.waterTintB ?? 0.04,
    ...
    const animate = () => {
      ...
      try {
        ...
        const newParams = validateShaderParams({
          ...
          waterTintR: cfg.waterTintR ?? 0.04,
          waterTintG: cfg.waterTintG ?? 0.04,
          waterTintB: cfg.waterTintB ?? 0.04,
